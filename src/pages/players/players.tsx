...
export default function PlayersPage () {
  ...
  const allPlayers = useLiveQuery(async () => {
    ...
    if (sort && !searching) {
      players = await db.players.orderBy('name').toArray() as Player[]
  ...
  const [players, score] = useMemo(()=>search(allPlayers || [], {keys: ['name', 'alias']}), [allPlayers, search])
  ...
  return (<PageContainer>
    <PageTitle id='players-title' title='Players'>
      ...
      <p>Players will be locked and cannot be removed if they've been matched in a game. Locked players can be marked absent for future rounds.</p>
    </PageTitle>
    ...
    <PageContent>
      ...
      {players.map(p=><PlayerInput key={p.id} player={p}/>)}
      <AddPlayerInput/>
    </PageContent>
    <PlayerProfileSheet/>
  </PageContainer>)
